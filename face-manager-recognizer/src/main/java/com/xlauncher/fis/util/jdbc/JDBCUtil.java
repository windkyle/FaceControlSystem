...
//        jdbcutil.getCount("2019-03-20 14:00:00", "2019-03-25 14:00:00");
    ...
    private void getImage(String startTime, String endTime, String path, int code) throws Exception {
        ...
        getCount(startTime, endTime, code);
        // 获取数据库连接
        ...
        if (code == 1) {
            ...
            // face_predict
            String sql = "SELECT predict_image FROM face_predict WHERE predict_time > ' " + startTime + "' AND predict_time < ' " + endTime + "'";
            try {
                // 获取Statement
            ...
            String sql = "SELECT user_card FROM syn_user WHERE model_calculation IS NOT NULL AND checkin_time > ' " + startTime + "' AND checkin_time < ' " + endTime + "'";
            try {
                // 获取Statement
                st = con.createStatement();
                // 执行查询操作
                ...
                while (rs.next()) {
                    count ++;
                    getImageByIdCard(rs.getString(1), path, count);
                }
                System.out.println("查询结束!");
            } catch (SQLException e) {
                e.printStackTrace();
    ...
    private int getCount(String startTime, String endTime, int code) throws Exception {
        ...
        if (code == 1) {
            // face_predict
            count = "SELECT COUNT(*) FROM face_predict WHERE predict_time > ' " + startTime + "' AND predict_time < ' " + endTime + "'";
        }  else if (code == 0) {
            // syn_user
            count = "SELECT COUNT(*) FROM syn_user WHERE model_calculation IS NOT NULL AND checkin_time > ' " + startTime + "' AND checkin_time < ' " + endTime + "'";
        }
        try {
            // 获取Statement
            st = con.createStatement();
